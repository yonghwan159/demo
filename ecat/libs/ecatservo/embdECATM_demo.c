...
int EcatInit(int cycleTime, int operationMode){
    ...
    if (!(slaveConf = ecrt_master_slave_config(eMaster, 
		...
		if (ecrt_slave_config_pdos(slaveConf, EC_END, slave_5_syncs)){
		       	fprintf(stderr, "Failed to configure out PDOs.\n");
        		return -1;
    		}
    	ecrt_slave_config_dc(slaveConf, 0x0700, cycleTime, 4400000, 0, 0);
    printf("OK!\n");

	// slaveConf = ecrt_master_slave_config(eMaster,ALIAS_POSITION(2), Beckhoff_EK1100);
	//     if (!slaveConf) {
	//         return -1;
	//     }

    // dome start beckoff 1100 2004 2024
	/*3.Slave Config */
	printf("\nConfiguring PDOs...");
	slaveConf = ecrt_master_slave_config(eMaster,ALIAS_POSITION(2), Beckhoff_EK1100);
    if (!slaveConf) {
	...
	/*3.Slave Config */
	printf("\nConfiguring PDOs...");
	// dome end beckoff 1100 2004 2024


	// for(i=2;i<LS_SLAVENUM+2;i++){
	// 	if (!(slaveConf = ecrt_master_slave_config(eMaster, 
	// 						ALIAS_POSITION(i),
	// 						LS_SERVO))){
 //        		fprintf(stderr, "Failed to get slave configuration.\n");
...
// 	slaveConf = ecrt_master_slave_config(eMaster,ALIAS_POSITION(6), Beckhoff_EK1100);
//     if (!slaveConf) {
//         return -1;
//     }
//     printf("\nConfiguring LCD_PDOs...");
...
/****************************************************************************/
void lsmecaFaultReset(int iNode)
{
	int i;
...
void lsmecaSetVelocity(int iNode, int RevPerMinute)
{
	...
	targetVelocity= (int)(LSMECA_19BIT_ENCRES * (float)(RPM2RPS(RevPerMinute)));
	// printf("target_pulse: %ld\n",targetVelocity);
	
	if (iNode == _ALLNODE){
		for(i=0;i<LS_SLAVENUM;i++){
			EC_WRITE_S32(eDomain_pd + lsmecaTargVelOff[i],targetVelocity);

		}	
	}
	else{
		EC_WRITE_S32(eDomain_pd + lsmecaTargVelOff[iNode],targetVelocity);
	}
}
/****************************************************************************/
void lsmecaSetAngularVelocity(int iNode, float AngularVelocity)
{
	...
	targetVelocity= (int)(LSMECA_19BIT_ENCRES * RADPS2RPS(AngularVelocity));
...
int lsmecaGetActualVelocityN(int iNode)
{
	int RevPerMinute = 0;
	float tmp;
	tmp = EC_READ_S32(eDomain_pd + lsmecaActlVelOff[iNode])/ LSMECA_19BIT_ENCRES;
	RevPerMinute = RPS2RPM(tmp); 
	//printf("get_pulse: %d\n", EC_READ_S32(eDomain_pd + lsmecaActlVelOff[iNode]));
	// return RevPerMinute;
	return RevPerMinute;
}
/****************************************************************************/
float lsmecaGetActualAngularVelocityN(int iNode)
{
	...
	currentVelocity = EC_READ_S32(eDomain_pd + lsmecaActlVelOff[iNode]);
...
{
	return  EC_READ_S32(eDomain_pd + lsmecaActlPosOff[iNode]);
}
/****************************************************************************/
void lsmecaSet(int iNode, int lcd)
{
	...
	if (iNode == _ALLNODE){
		for(i=1;i<2;i++){
			EC_WRITE_U8(eDomain_pd + lsmeca[i],lcd);

		}
		for(i=LS_SLAVENUM+1;i<LSMECAPION_SLAVENUM;i++){
			EC_WRITE_U8(eDomain_pd + lsmeca[i],lcd);
		}

	}
	else{
		EC_WRITE_U8(eDomain_pd + lsmeca[iNode],lcd);
	}
}
/****************************************************************************/
void lsmecaSet1(int iNode, int lcd)
{
	int i;
	...
	if (iNode == _ALLNODE){
		...
		for(i=LS_SLAVENUM+4;i<LSMECAPION_SLAVENUM;i++){
			EC_WRITE_U8(eDomain_pd + lsmeca1[i],lcd);
		}

	}
	else{
		EC_WRITE_U8(eDomain_pd + lsmeca1[iNode],lcd);
	}
}